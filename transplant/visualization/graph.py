...
def plot_dynamic_features(df, id_patient, features_list,
    ...
    if len(features_list) != 0:
        ...
        features_list = [feature for feature in features_list
                         if feature not in(['time', 'id_patient'])]
...
def plot_compare_patient(df, feature_to_analyse, patient_list):
    ...
    if not isinstance(feature_to_analyse, str):
        raise Exception("""feature_to_analyse muse be a string - \
                        Example 'ETCO2' """)
    ...
    if not isinstance(patient_list, list):
        raise Exception("""'patient_list' must be a list - \
                        Example [304, 305, 405]""")
...
def plot_analyse_factory(df, pca, hue=False, kmean=None):
    ...
    # Design graph
    layout = dict(
        title='Analyse de feature PCA explain (first 2 components) : ' +
        str(pca_expl),
        height=400,
        xaxis=dict(
            title='pca_1'
